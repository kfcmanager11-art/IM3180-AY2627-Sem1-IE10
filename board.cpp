...
Board::Board() {
    ...
    for(auto &i: current_board){
        for(auto& j: i){
            board_hash ^= piece_hash[j * 64 + (&j - &i[0]) + (&i - &current_board[0]) * 8];
        }
    }
...
auto Board::begin() -> std::vector<std::vector<int>>::iterator{ return current_board.begin(); }
auto Board::end() -> std::vector<std::vector<int>>::iterator{ return current_board.end(); }
auto Board::cbegin() const -> std::vector<std::vector<int>>::const_iterator{ return current_board.begin(); }
auto Board::cend() const -> std::vector<std::vector<int>>::const_iterator{ return current_board.end(); }

bool Board::inboard(int x, int y) const { return (x >= 0 && y >= 0 && x < 8 && y < 8); }
...
int Board::get_piece(int r, int c) const { return current_board[r][c]; }

int Board::get_piece_value(int piece) const {
    auto it = piece_value.find(piece);
    return (it != piece_value.end()) ? it->second : 0;
}

const std::vector<std::pair<int, int>>& Board::get_move_patterns(int piece) const {
    static const std::vector<std::pair<int, int>> empty;
    auto it = move_pattern.find(piece);
    return (it != move_pattern.end()) ? it->second : empty;
}

const std::vector<std::pair<int, int>>& Board::get_pawn_move_patterns(int piece) const {
    static const std::vector<std::pair<int, int>> empty;
    auto it = pawn_move_pattern.find(piece);
    return (it != pawn_move_pattern.end()) ? it->second : empty;
}

const std::vector<std::pair<int, int>>& Board::get_pawn_capture_patterns(int piece) const {
    static const std::vector<std::pair<int, int>> empty;
    auto it = pawn_capture_pattern.find(piece);
    return (it != pawn_capture_pattern.end()) ? it->second : empty;
}

void Board::get_turn(){
    if(!turn) std::cout << "White turn\n";
...
bool Board::valid_move(int old_x, int old_y, int new_x, int new_y){
    ...
    if(piece == 1 || piece == -1) {
        if(destination_piece == 0) {
            ...
            for(auto& move : moves) {
                if(move.first == dx && move.second == dy) return true;
            }
            return false;
        }
        else {
            ...
            for(auto& capture : captures) {
                if(capture.first == dx && capture.second == dy) return true;
            }
            return false;
        }
    }
    ...
    for(auto& pattern : patterns) {
        if(pattern.first == dx && pattern.second == dy) return true;
    }
...
bool Board::make_move(int old_x, int old_y, int new_x, int new_y){
    ...
    rollback.push_back(std::make_tuple(old_x, old_y, new_x, new_y, current_board[new_x][new_y], this->turn, this->move_left));
...
bool Board::make_capture(int old_x, int old_y, int new_x, int new_y){
    ...
    rollback.push_back(std::make_tuple(old_x, old_y, new_x, new_y, captured_piece, this->turn, this->move_left));
